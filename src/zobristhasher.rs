...
use chess::{BitBoard, Board, BoardStatus, ChessMove, MoveGen, Rank, Square};



const RANDOM_VALUES_POLYGLOT_PIECE: [u64; 768] = [
...
const RANDOM_VALUES_POLYGLOT_CASTLE: [u64; 4] = [
    0x31D71DCE64B2C310, 0xF165B587DF898190, 0xA57E6339DD2CF3A0, 0x1EF6E6DBB1961EC9,
];

const RANDOM_VALUES_POLYGLOT_ENPASSANT: [u64; 8] = [
    0x70CC73D90BC26E24, 0xE21A6B35DF0C3AD7, 0x003A93D8B2806962, 0x1C99DED33CB890A1,
    0xCF3145DE0ADD4289, 0xD0E4427A5514FB72, 0x77C621CC9FB3A483, 0x67A34DAC4356550B,
];

const RANDOM_VALUES_POLYGLOT_TURN: [u64; 1] = [0xF8D626AAAF278509];
...
impl ZobristHasher{
    ...
    pub fn hash_board(&self, board: &Board) -> u64{
        //following the zobrist hash spec I found here http://hgm.nubati.net/book_format.html
        

        let mut piece_key: u64 = 0;
        let mut castle_key: u64 = 0;
        let mut enpassant_key: u64 = 0;
        let mut turn_key: u64 = 0;
        ...
        for rank in 0..8{
            for file in 0..8{
                ...
                match board.piece_on(square){
                    Some(piece) => {
                        let mut kind_of_piece = 0;
                        let piece_color = board.color_on(square).unwrap_or(chess::Color::White);//its gauranteed to have a color anyways
                        
                        kind_of_piece = match piece{
                            chess::Piece::Pawn => 0,
                        ...
                        let offset_piece = 64*kind_of_piece + 8*rank + file;
                        piece_key = piece_key ^ RANDOM_VALUES_POLYGLOT_PIECE[offset_piece];
                    }
                    None => {}
                }
            }
        ...
        if board.castle_rights(chess::Color::White) == chess::CastleRights::NoRights && board.castle_rights(chess::Color::Black) == chess::CastleRights::NoRights{
            castle_key = 0;
        }
        else{
            match board.castle_rights(chess::Color::White){
                chess::CastleRights::KingSide => {
                    castle_key = castle_key ^ RANDOM_VALUES_POLYGLOT_CASTLE[0];
                }
                chess::CastleRights::QueenSide => {
                    castle_key = castle_key ^ RANDOM_VALUES_POLYGLOT_CASTLE[1];
                }
                chess::CastleRights::Both => {
                    castle_key = castle_key ^ RANDOM_VALUES_POLYGLOT_CASTLE[0];
                    castle_key = castle_key ^ RANDOM_VALUES_POLYGLOT_CASTLE[1];
                }
                chess::CastleRights::NoRights => {}//do nothing, we already handled the both colors none case above
            }

            match board.castle_rights(chess::Color::Black){
                chess::CastleRights::KingSide => {
                    castle_key = castle_key ^ RANDOM_VALUES_POLYGLOT_CASTLE[2];
                }
                chess::CastleRights::QueenSide => {
                    castle_key = castle_key ^ RANDOM_VALUES_POLYGLOT_CASTLE[3];
                }
                chess::CastleRights::Both => {
                    castle_key = castle_key ^ RANDOM_VALUES_POLYGLOT_CASTLE[2];
                    castle_key = castle_key ^ RANDOM_VALUES_POLYGLOT_CASTLE[3];
        ...
        match board.en_passant(){
            Some(sq) => {
                ...
                enpassant_key = enpassant_key ^ RANDOM_VALUES_POLYGLOT_ENPASSANT[file_idx];
            }
            None => {
                enpassant_key = 0;
            }
        ...
        match board.side_to_move(){
            chess::Color::White => {
                turn_key = RANDOM_VALUES_POLYGLOT_TURN[0];
            }
            chess::Color::Black => {
                turn_key = 0;
            }
        ...
        let key: u64 = piece_key ^ castle_key ^ enpassant_key ^ turn_key;
